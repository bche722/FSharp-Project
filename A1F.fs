...
[<EntryPoint>]
let main args = 
    let path1 = args.[0].[4..(args.[0].Length-1)]
    let path2 = args.[1].[4..(args.[1].Length-1)]
    let str1 = File.ReadAllText(path1)
    let str2 = File.ReadAllText(path2)
    if String.Equals(args.[2], "/SEQ") then 
        (seq str1 str2) |> printfn "1 1 %d"
    else 
        if String.Equals(args.[2].[0..3], "/ACT") then
            let a = args.[2].[5..(args.[2].Length-1)].Split [|','|]
            if (int a.[2])=0 then duration <| fun () -> act str1 str2 (int a.[0]) (int a.[1]) |> printfn "%d %d %d" (int a.[0] - 1) (int a.[1] - 1)
            elif (int a.[2])=1 then
                duration <| fun () -> act str1 str2 (int a.[0]) (int a.[1]) |>ignore
                for  i1 = 0 to (int a.[0])-1 do
                    for i2 = 0 to (int a.[1])-1 do
                        tempResult.[i1,i2].Task.Result |> printfn "%d %d %d" i1 i2
        elif String.Equals(args.[2].[0..3], "/CSP") then
            let a = args.[2].[5..(args.[2].Length-1)].Split [|','|]
            if (int a.[2])=0 then duration <| fun () -> csp str1 str2 (int a.[0]) (int a.[1]) |> printfn "%d %d %d" (int a.[0] - 1) (int a.[1] - 1)
            elif (int a.[2])=1 then
                duration <| fun () -> csp str1 str2 (int a.[0]) (int a.[1]) |>ignore
                for  i1 = 0 to (int a.[0])-1 do
                    for i2 = 0 to (int a.[1])-1 do
                        tempResult.[i1,i2].Task.Result |> printfn "%d %d %d" i1 i2
    0    
